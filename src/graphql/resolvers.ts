    ...
    Mutation: {
        async cohortInviteMember(_parent, { input }, { cohortAdapter }) {
            try {
                ...
                const existingInvite = await cohortAdapter.findInviteByEmail(
                    input.email,
                );
                ...
                const existingMember = await cohortAdapter.findMemberByEmail(
                    input.email,
                );
                ...
                await cohortAdapter.onUnexpectedError(
                    UnexpectedError.wrapForResolver(
                        "Mutation.cohortInviteMember",
                        err,
                    ),
                );
                return {

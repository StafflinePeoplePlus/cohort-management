...
export class UnexpectedError extends Error {
    ...
    static wrapForResolver(resolver: string, err: unknown): UnexpectedError {
        return err instanceof UnexpectedError
            ? err
            : new UnexpectedError(
                  `Unexpected error in resolver of ${resolver}`,
                  err,
              );
    }
}
...
export class SendInviteFailed extends UnexpectedError {
    constructor(public inviteID: string, public email: string, cause: unknown) {
        super(
            `Failed to sent invite email out to \`${email}\` for invite ID \`${inviteID}\``,
            cause,
        );
    }
}
